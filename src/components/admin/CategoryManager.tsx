    ...
    <div className="flex flex-col lg:flex-row h-full">
      ...
      <div className="w-full lg:w-1/2 p-6 flex flex-col h-full overflow-hidden bg-white">
        <div className="mb-6 shrink-0">
          <div className="relative">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 pointer-events-none" size={20} />
            <input 
              type="text" 
              value={search}
        ...
        <div className="flex flex-col flex-grow border border-gray-100 rounded-xl bg-gray-50/50 overflow-hidden shadow-sm">
          <div className="flex-grow overflow-y-auto max-h-[500px] p-2">
            {filteredCategories.length === 0 ? (
              <div className="text-center p-8 text-gray-400 font-medium text-sm">No se encontraron categorías.</div>
            ) : (
      ...
      <div className="w-full lg:w-1/2 p-6 lg:p-8 bg-gray-50/30 flex flex-col border-t lg:border-t-0 lg:border-l border-gray-100">
        <div className="flex items-center justify-between mb-6 shrink-0">
          ...
          {editingId && (
            <button type="button" onClick={resetForm} className="text-xs font-bold text-primary hover:text-foreground flex items-center gap-1 uppercase tracking-widest px-3 py-1.5 bg-primary/10 rounded-full transition-colors">
        ...
        <form onSubmit={handleSave} className="flex flex-col gap-5 flex-grow">
          <div>
            <label className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-1.5 block">Nombre Corto</label>
            <input 
              type="text" 
              required
              value={nombre}
          ...
          <div className="mt-4 shrink-0 mt-auto pt-6">
            ...
            >
              {loading ? "Procesando..." : (editingId ? "Guardar Cambios" : "Agregar Categoría")}
            </button>
            <div className="h-6 mt-3 text-center">
               {msg && <span className={`text-xs font-bold ${msg.includes("Error") ? "text-red-500" : "text-green-500"}`}>{msg}</span>}
            </div>
          </div>
        </form>
      </div>
