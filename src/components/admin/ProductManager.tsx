...
export default function ProductManager() {
  ...
  const handleSaveProduct = async (e: React.FormEvent) => {
    ...
    if (file && currentId) {
      try {
        setMsg(
          editingId
            ? "Actualizando producto y procesando imagen..."
            : "Producto creado resolviendo imagen...",
        );
  ...
  const executeDeleteImage = async () => {
    ...
    await supabase.storage
      .from("imagenes")
      .remove([`productos/${editingId}.jpg`, `productos/${editingId}.webp`]);
  ...
  const executeDeleteProduct = async (id: number) => {
    ...
    await supabase.storage
      .from("imagenes")
      .remove([`productos/${id}.jpg`, `productos/${id}.webp`]);
      ...
      <div className="bg-white rounded-3xl shadow-sm border border-gray-100 mt-8 overflow-hidden flex flex-col lg:flex-row">
              ...
              {prodState === "error" && (
                <div className="flex flex-col items-center justify-center p-12 text-red-500 bg-red-50 rounded-xl m-2 border border-red-100">
                  ...
                  <button
                    ...
                    className="flex items-center gap-2 bg-white text-red-600 px-4 py-2 rounded-lg font-bold border border-red-200 hover:bg-red-600 hover:text-white transition-colors"
                  >
                    ...
                    >
                      ...
                      >
                        <div className="w-12 h-12 bg-white rounded-md border border-gray-100 flex items-center justify-center relative overflow-hidden shrink-0">
                          {prod.image_url ? (
                            ...
                            <img
                              ...
                              onError={(e) => {
                                if (
                                  prod.image_url_fallback &&
                                  !e.currentTarget.src.includes(".jpg")
                                ) {
                        ...
                        <div>
                          ...
                          <div className="flex items-center gap-1.5 mt-1 overflow-hidden w-full">
                            <p className={`text-[10px] font-black uppercase shrink-0 ${prod.activo && isCatActive ? "text-primary" : "text-gray-400"}`}>
        ...
        <div className="w-full lg:w-1/2 p-6 lg:p-8 bg-gray-50/30 flex flex-col">
          ...
          >
              ...
              {editingId && (
                <div className="flex flex-col gap-2 mt-3 p-4 bg-white border border-gray-100 rounded-xl">
                  ...
                  <p className="text-[11px] text-gray-400 font-medium">
                    Nota: Subir una imagen reemplazará automáticamente la
                    actual.
                  </p>
                </div>
              )}
            ...
            {msg && (
              <div
                className={`p-3 rounded-lg text-sm font-bold text-center border ${msg.includes("Error") ? "bg-red-50 text-red-600 border-red-100" : "bg-green-50 text-green-600 border-green-100"}`}
              >
                {msg}
              </div>
            )}
