...
export default function ProductGrid() {
  ...
  const loadProducts = useCallback(async () => {
    ...
    if (error) {
      setErrorMsg(
        "Ocurrió un problema al cargar los productos. Por favor, reintenta en unos instantes.",
      );
    }
    ...
    <div className="flex flex-col md:flex-row gap-8">
      ...
      <aside className="w-full md:w-64 flex-shrink-0 flex flex-col gap-6">
        <div className="bg-white p-5 rounded-3xl shadow-sm border border-gray-100 flex flex-col gap-4 sticky top-[100px]">
          ...
          {debugMsg && (
            <p className="text-red-500 text-xs font-bold border border-red-500 p-2 rounded-lg">
              {debugMsg}
            </p>
          )}
          <h3 className="font-heading font-black text-lg text-foreground mt-4">
            Categorías
          </h3>
      ...
      <main
        id="productos-grid"
        className="flex-1 flex flex-col gap-8 scroll-mt-24"
      >
        {/* Results Info */}
        ...
        {errorMsg ? (
          <div className="w-full bg-red-50 text-red-600 p-8 rounded-3xl border border-red-100 flex flex-col items-center justify-center min-h-[300px]">
            ...
            <h3 className="font-heading font-black text-xl mb-2 text-center">
              Algo salió mal
            </h3>
            <p className="text-sm font-medium text-center max-w-md opacity-80 mb-6">
              {errorMsg}
            </p>
            <button
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-4 md:gap-6">
            {Array.from({ length: 12 }).map((_, i) => (
              <div
                key={i}
                className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden flex flex-col h-full opacity-60"
              >
          ...
          <div className="w-full min-h-[300px] flex flex-col items-center justify-center bg-white rounded-3xl border border-gray-100 p-8 text-center animate-in fade-in zoom-in-95 duration-500 shadow-sm">
            ...
            <h3 className="font-heading text-2xl font-black text-gray-800 mb-2">
              No encontramos nada
            </h3>
            <p className="text-gray-500 font-medium max-w-md">
              No hay productos que coincidan con &quot;
              {searchQuery || selectedCategory}&quot;. Intenta con otra búsqueda
              o selecciona &quot;Todas las categorías&quot;.
            </p>
        ...
        <div className="fixed inset-0 z-[110] flex items-end md:items-center justify-center p-4 md:p-6">
          ...
          <div className="relative w-full max-w-3xl bg-white md:rounded-3xl rounded-t-3xl max-h-[100dvh] md:max-h-[90vh] shadow-2xl overflow-y-auto overflow-x-hidden flex flex-col md:flex-row animate-in slide-in-from-bottom-10 md:zoom-in-95 duration-300">
            ...
            <div className="w-full md:w-1/2 aspect-square md:min-h-[400px] bg-white relative border-b md:border-b-0 md:border-r border-gray-100 flex items-center justify-center p-4 md:p-8 shrink-0">
              ...
              {/* Fallback */}
              <div
                className={`flex flex-col items-center justify-center text-gray-300 border-2 border-dashed border-gray-100 rounded-2xl absolute inset-6 bg-white ${selectedProduct.image_url && !modalImageError ? "hidden" : ""}`}
              >
