...
const PDFButton = dynamic(() => import("@/components/admin/PDFButton"), {
  ...
  loading: () => (
    <button
      disabled
      className="w-full flex items-center justify-center gap-3 py-4 bg-gray-200 text-gray-400 rounded-xl font-bold uppercase tracking-widest text-sm"
    >
...
export default function PresupuestosPage() {
      ...
      if (exists) {
        return prev.map((i) =>
          i.producto_id === prod.id
            ? { ...i, cantidad: i.cantidad + 1, precio: precioBase }
            : i,
        );
      }
      return [
  ...
  const updateQuantity = (id: number, qty: number) => {
    if (qty < 1) return;
    setItems(
      items.map((i) => (i.producto_id === id ? { ...i, cantidad: qty } : i)),
    );
  };
  ...
  const fileName = useMemo(() => {
    ...
    const clientSanitized = cliente
      ? cliente.replace(/\s+/g, "-").substring(0, 20)
      : "Cliente";
  ...
  const wppText = useMemo(() => {
    const productosTexto = items
      .map((i) => {
        const catTexto =
          i.categoria && i.categoria !== "General" ? `${i.categoria} - ` : "";
    ...
    <div className="max-w-7xl mx-auto flex flex-col xl:flex-row gap-8 animate-in fade-in duration-500 pb-20">
      <div className="w-full xl:w-5/12 flex flex-col gap-6">
        <div className="bg-white p-6 md:p-8 rounded-[2rem] shadow-sm border border-gray-100 flex flex-col gap-6">
          <div>
            <h1 className="text-2xl font-black font-heading tracking-tight mb-1">
              Generador de Presupuestos
            </h1>
            <p className="text-sm text-gray-500 font-medium">
              Arma la lista y expórtala en PDF en segundos.
            </p>
          </div>
                ...
                <div className="relative group">
                  <select
                    value={
                      localidad === ""
                        ? ""
                        : localidadesPredefinidas.includes(localidad)
                          ? localidad
                          : "otra"
                    }
                    onChange={(e) => {
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <select
                    ...
                    onChange={(e) =>
                      setAjusteTipo(
                        e.target.value as "ninguno" | "descuento" | "recargo",
                      )
                    }
          ...
          {items.length === 0 ? (
            <div className="flex-grow flex flex-col items-center justify-center text-gray-300 gap-4 py-20 border-2 border-dashed border-gray-100 rounded-2xl">
              ...
              <p className="font-medium text-sm text-center px-4">
                Busca productos a la izquierda para armar el presupuesto.
              </p>
            </div>
          ) : (
                ...
                <table className="w-full text-left border-collapse">
                  <thead>
                    <tr className="border-b-2 border-gray-900">
                      <th className="py-3 px-2 text-xs font-black uppercase tracking-widest text-gray-400 w-1/2">
                        Producto
                      </th>
                      <th className="py-3 px-2 text-xs font-black uppercase tracking-widest text-gray-400">
                        Cant
                      </th>
                      <th className="py-3 px-2 text-xs font-black uppercase tracking-widest text-gray-400 text-right">
                        Subtotal
                      </th>
                    ...
                    {items.map((item) => (
                      ...
                      >
                        <td className="py-4 px-2">
                          <p className="text-sm font-bold text-foreground leading-snug pr-4">
                            {item.nombre}
                          </p>
              ...
              <div className="mt-auto pt-6 md:pt-8 flex flex-col gap-4 bg-white border-t border-gray-100 px-4 pb-[env(safe-area-inset-bottom,1.5rem)] md:pb-0 md:border-t-0 md:bg-transparent md:px-0">
                  ...
                  {ajusteTipo !== "ninguno" && (
                    <div className="flex justify-between items-center text-primary">
                      <span className="text-xs font-bold uppercase tracking-widest">
                        {ajusteTipo === "descuento" ? "Descuento" : "Recargo"} (
                        {ajustePorcentaje}%)
                      </span>
                      <span className="text-sm font-black">
                        {ajusteTipo === "descuento" ? "-" : "+"}$
                        {Math.abs(ajusteValor).toLocaleString("es-AR")}
                      </span>
                    </div>
                  )}
                </div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-2 gap-3 mt-4 md:mt-6">
                  {!cliente.trim() ? (
                    ...
                    <PDFButton data={budgetData} fileName={fileName} />
                  )}
